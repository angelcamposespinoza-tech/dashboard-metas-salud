...
if archivos:
    # --- SECCIÓN DE AUDITORÍA EN BARRA LATERAL ---
    st.sidebar.divider()
    ...
    if st.sidebar.button("🔍 Generar Lista de Unidades Pendientes"):
        if not periodo_sel: st.error("Seleccione un periodo.")
        else:
            st.header(f"📋 Unidades con Carga Pendiente ({', '.join(periodo_sel)})")
            reporte_omisiones = []
            for arc in archivos:
            ...
            if reporte_omisiones:
                ...
                st.download_button("📥 Descargar Lista en PDF", pdf_b, "lista_unidades.pdf", "application/pdf")
            else: st.success("✅ No hay unidades pendientes.")

    # BOTÓN 2: REPORTE DETALLADO
    if st.sidebar.button("📑 Mostrar rubros detallados no registrados"):
        if not periodo_sel: st.error("Seleccione un periodo.")
        else:
            st.header(f"⚠️ Detalle de Indicadores sin Logro")
            reporte_full = []
            for arc in archivos:
                dict_temp = pd.read_excel(arc, sheet_name=None, header=None)
                for nombre_hoja, df_hoja in dict_temp.items():
            ...
            if reporte_full:
                ...
                st.download_button("📥 Descargar Detalle (Excel)", out_ex.getvalue(), "detalle_omisiones.xlsx")
            else: st.success("✅ Todos los rubros están llenos.")
